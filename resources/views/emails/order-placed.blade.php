...
| {{ $item->product_name }} | {{ $item->quantity }} | {{ number_format($item->unit_price * $item->quantity, 0, '.', '.') }}đ |
@endforeach
...
**Phương thức thanh toán:** {{ $order->payment_method == 'cod' ? 'Thanh toán khi nhận hàng (COD)' : 'Ví Momo' }}
